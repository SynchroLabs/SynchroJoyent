...
exports.getMachineAudit = function * (context, dataCenter, id, noCache)
{
    var client = getSdcClient(context, dataCenter);

    var auditResult = yield Synchro.yieldAwaitable(context, function(callback)
    {
        client.getMachineAudit(id, callback);
    });

    var audit = auditResult[0];

    console.log("Got audit:", audit);
